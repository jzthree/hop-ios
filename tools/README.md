...
| `git stash push -- <file>` on a file with NO uncommitted changes creates NO stash, and the "discriminator" then re-tests identical code | a bisect that "proved" a change innocent had never reverted anything — the change was already committed | check `git status` (or that `stash pop` finds an entry) before trusting a revert-and-rerun; better, run the suspect test against committed HEAD directly |
| the same wall tests fail in a DIFFERENT combination every run after ~10 suites on one simulator | progressive sim degradation, not a moving regression — a fixed regression fails the same test every time | reboot the sim and re-run BEFORE bisecting; a changing failure set is the tell |
| in a SwiftUI Menu, a Toggle is a BUTTON, not a switch | `app.switches["Tile view"]` matched nothing, the else-branch dismissed the menu, and the probe shot the LIST while reporting it had shot tiles | query `app.buttons[label]`; and since the preference is UserDefaults-sticky, assert what you toggled INTO instead of trusting the tap |
